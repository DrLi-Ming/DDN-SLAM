...
KeyFrame::KeyFrame():
        ...
        NLeft(0),NRight(0), mnNumberOfOpt(0), mbHasVelocity(false), mpNerfXform(NULL)
{
...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB):
    ...
    mvKeysRight(F.mvKeysRight), NLeft(F.Nleft), NRight(F.Nright), mTrl(F.GetRelativePoseTrl()), mnNumberOfOpt(0), mbHasVelocity(false), mpNerfXform(NULL)
{
    mnId=nNextId++;
    ...
    mImuBias = F.mImuBias;
    SetPose(F.GetPose());
    SetColorImage(F.GetColorImage()); 
    SetDepthImage(F.GetDepthImage());
...
void KeyFrame::SetPose(const Sophus::SE3f &Tcw)
{
    ...
    if (mpNerfXform){
        // for (int m = 0; m < 3; ++m) {
        //     for (int n = 0; n < 4; ++n) {
        //         mpNerfXform->start(m, n) = mTwc(m,n);
        //         mpNerfXform->end(m, n)   = mTwc(m,n);
        //     }
        // }
        mpNerfXform->start = mTwc.matrix().block<3,4>(0,0);
        mpNerfXform->end = mTwc.matrix().block<3,4>(0,0);
...
Eigen::Matrix<float, 3, 4> KeyFrame::GetPoseWithPhotometric()
{
    unique_lock<mutex> lock(mMutexPose);
    if (mpNerfXform){
        return mpMap->PoseWithPhotometric(mpNerfIndex);
    }
    else{
        throw std::runtime_error({"no mpNerfXform initialized yet."});
    }
}
...
void KeyFrame::SetColorImage(const cv::Mat &imColor){
    mImColor = imColor;
}
...
void KeyFrame::SetDepthImage(const cv::Mat &imDepth){
    mImDepth = imDepth;
}

cv::Mat KeyFrame::GetDepthImage() {
    return mImDepth;
}
