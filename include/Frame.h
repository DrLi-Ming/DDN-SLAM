...
public:
    ...
    void RemoveMovingKeyPoints(const cv::Mat &ImGray, const cv::Mat &ImDepth = cv::Mat());
    ...
    void RemoveMovingPointsWithEpiAndYOLO(std::vector<double> epiErr, const cv::Mat &ImGray, const cv::Mat &ImDepth = cv::Mat());
...
public:
    ...
    cv::Mat mImColor;
    cv::Mat mImDepth;
    cv::Mat Pixeldynamic;  // 使用cv::Mat存储像素数据
    ...
    Frame(Tracking* pTracker, const cv::Mat &imLeft, const cv::Mat &imRight, const double &timeStamp, ORBextractor* extractorLeft, ORBextractor* extractorRight, ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, GeometricCamera* pCamera, GeometricCamera* pCamera2, Sophus::SE3f& Tlr,Frame* pPrevF = static_cast<Frame*>(NULL), const IMU::Calib &ImuCalib = IMU::Calib());
